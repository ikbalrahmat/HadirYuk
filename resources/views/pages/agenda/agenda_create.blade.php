...
<div class="container py-4">
    <h2 class="mb-4">Tambah Item Agenda untuk Rapat: {{ $rapat->nama_rapat }}</h2>
    ...
    <form action="{{ route('agenda.items.store', $rapat->id) }}" method="POST">
        @csrf

        <div class="mb-3">
            <label for="jam" class="form-label">Jam</label>
            <input type="text" name="jam" id="jam" class="form-control" value="{{ old('jam') }}" required>
        </div>

        <div class="mb-3">
            <label for="agenda" class="form-label">Agenda</label>
            <textarea name="agenda" id="agenda" class="form-control" rows="4" required>{{ old('agenda') }}</textarea>
        </div>
